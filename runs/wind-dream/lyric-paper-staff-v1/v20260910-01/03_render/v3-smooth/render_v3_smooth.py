...
def main():
    ...
    if args.preview:
        for t in (4.42,4.62,4.88,7.28,7.5,7.82,16.86,17.14,17.44): frame(round(t*FPS),cues).save(OUT/f'preview-{t:.2f}.jpg',quality=90)
        print('preview pass'); return
    dest=OUT/'wind-dream-v3-smooth.mp4'; cmd=[str(FFMPEG),'-y','-v','error','-f','rawvideo','-pix_fmt','rgb24','-s',f'{W}x{H}','-r',str(FPS),'-i','-','-ss',str(START),'-i',str(AUDIO),'-t',str(DURATION),'-map','0:v','-map','1:a:0','-c:v','libx264','-preset','veryfast','-crf','18','-pix_fmt','yuv420p','-c:a','aac','-b:a','192k','-movflags','+faststart',str(dest)]
    p=subprocess.Popen(cmd,stdin=subprocess.PIPE)
    for n in range(N): p.stdin.write(frame(n,cues).tobytes())
    p.stdin.close(); assert p.wait()==0
    subprocess.run([str(FFMPEG),'-v','error','-i',str(dest),'-f','null','-'],check=True); meta=probe(dest)
    (OUT/'v3-smooth-result.json').write_text(json.dumps({'design_brief':{'job':'22-second lyric motion review','priority':'continuous readable lyrics','removed':'character lift/scale/bob, spectrum, notes, particles','signature':'paper-wind white cursor across static LongCang gold'},'parameters':{'fps':FPS,'size':[W,H],'highlight':'raised cosine, #FFF9E7','crossfade':CROSSFADE,'background_drift_px':[4,3],'SPECTRUM':False,'NOTES':False,'PARTICLES':False},'inputs':{'alignment':str(ALIGN),'audio':str(AUDIO),'background':str(BG)},'output':{'path':str(dest),'sha256':hashlib.sha256(dest.read_bytes()).hexdigest(),'media':meta},'checks':{'decode':'pass','audit':audit_data},'untested':['manual listening review','full-song render']},ensure_ascii=False,indent=2),encoding='utf-8')
    print(dest)
